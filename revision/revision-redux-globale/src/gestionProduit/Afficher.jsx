...
import { ModifieProduit, SupprimerProduit } from "../redux/action/actionProduit";
...
    },[])
    ...
    function supprimer(id){
        return dispatch(SupprimerProduit(id))
    }

    function modifier(){
        
        return dispatch(ModifieProduit({...produit}))
    }
        ...
        <div class="min-w-full inline-block align-middle">
            ...
            <div class="overflow-hidden ">
                <table class=" min-w-full rounded-xl">
                    <thead>
                        <tr class="bg-gray-50">
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl"> # </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Nom </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> categorie </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Quantite  </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Prix  </th>
                    ...
                    <tbody class="divide-y divide-gray-300 ">
                        {produits.map(function(item,index){
                            return(
                                <tr class="bg-white transition-all duration-500 hover:bg-gray-50" key={index}>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 "> {item.id}</td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {item.nom} </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {item.categorie.libelle} </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{item.quantite} </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {item.prix}</td>
                                    <td class=" p-5 ">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <button class="p-2 rounded-full  group transition-all duration-500  flex item-center" onClick={()=>supprimer(item.id)}>
...
<div class="p-8">
  ...
  <input type="number" name="prix" class="mt-1 block mb-10  w-full w-1/2 rounded-md border border-slate-300 bg-white px-3 py-4 placeholder-slate-400 shadow-sm placeholder:font-semibold placeholder:text-gray-500 focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 sm:text-sm"  placeholder="Prix *" value={produit.prix}  onChange={(e)=>setProduit({...produit,prix:e.target.value})}/>
  </div>
  <div class="text-center">
          <a class="cursor-pointer rounded-lg bg-blue-700 px-8 py-5 text-sm font-semibold text-white" onClick={modifier}>Book Appoinment</a>
  </div>
</div>
</div>
        </>
