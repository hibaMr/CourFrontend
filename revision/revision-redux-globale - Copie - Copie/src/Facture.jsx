...
function Facture(){
  ...
  function getProduitByCode(code){
    return produits.find(function(item){
      return item.codeProduit == code
    })
  }

  function filtere (){
    const _clients = clients.filter(function(item){
      return item.numero == idF
    })
    const _achats = achats.map(function(item){
      if(item.numero == idF){
        const _produits = getProduitByCode(item.codeProduit)
        return {...item,intitule:_produits.intitule,prix:_produits.prix,qteStockees:_produits.qteStockees}
      }
      return item
    })
    setFacture([...facture,{numero:_clients.numero,nom:_clients.nom,prenom:_clients.prenom,achats:[..._achats]}])
  }
...
<div class="text-center m-8">
      ...
      <a class="cursor-pointer rounded-lg bg-blue-700 px-8 py-5 text-sm font-semibold text-white" onClick={filtere}>Filtrer</a>
      ...
      <div class=" overflow-x-auto">
        <div class="min-w-full inline-block align-middle">
            <div class="relative  text-gray-500 focus-within:text-gray-900 mb-4">
                ...
                <input type="text" id="default-search" class="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search for company" onChange={(e)=>setIdF(e.target.value)}/>
            </div>
                ...
                <table class=" min-w-full rounded-xl">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> intitule </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Quantite </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Quantite  stock </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Prix  </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Total  </th>
                            <th scope="col" class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl"> Actions </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-300 ">
                        {facture.map(function(item,index){
                          return(
                            <tr class="bg-white transition-all duration-500 hover:bg-gray-50" key={index}>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">{item.codeProduit} </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{item.intitule} </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{item.qte}  </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{item.qteStockees} </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{item.prix} </td>
                                    <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{item.prix * item.qte} </td>
                                    <td class=" p-5 ">
